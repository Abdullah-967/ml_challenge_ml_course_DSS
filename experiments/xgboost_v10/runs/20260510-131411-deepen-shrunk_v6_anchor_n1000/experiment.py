"""xgboost_v10 deepen iter1 -- runtime-safe shrunk v6 anchor (no transfer).

Lane 3 (smoke iter3) timed out at the 10-min hard stop with n_est=2000 +
transfer features. Re-anchor here at n_est=1000 (single capacity knob) so
deepen iter2 (which adds the 4 transfer features) finishes inside budget.
This iter establishes the n=1000 baseline; the next iter measures the
transfer-feature lift cleanly.
"""
...
RECIPE_VARIANT = "deepen_iter1_anchor_n1000"
PARAMS_SUMMARY = (
