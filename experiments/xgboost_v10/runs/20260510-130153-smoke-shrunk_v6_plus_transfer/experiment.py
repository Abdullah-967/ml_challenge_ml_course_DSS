"""xgboost_v10 smoke iter3 -- shrunk v6 anchor + transfer-mode features.

Single-knob change vs Lane 2: add four deterministic features derived from
parcel_ids vs transferred_parcel_ids set arithmetic:
  - n_transferred (size of transferred set)
  - n_parcels_kept (intersection size)
  - transfer_overlap_ratio (|inter| / |union|)
  - is_no_transfer (1 if transferred set is empty)

Train-set inspection: 32% of rows are no_transfer (median price 80k vs 118k
for full_match), and no_transfer correlates with future_sale at 11.5% vs 1.0%
baseline -- mostly off-plan sales. The signal is structurally orthogonal to
size/area features. Compare this run's MAE to Lane 2 (48441.69) to measure
the transfer-feature lift.
"""
...
RECIPE_VARIANT = "smoke_iter3_shrunk_v6_plus_transfer"
PARAMS_SUMMARY = (
    "XGBRegressor(reg:absoluteerror,n_est=2000,lr=0.05,max_depth=6,"
    "mcw=5,subsample=0.8,colsample=0.6,reg_lambda=1.0,hist,enable_categorical=True),"
    "0.7*global+0.3*submodel(top5_types_min200,n_est=2000),log1p,"
...
NUMERIC_COLUMNS = (
    ...
    "built_per_land",
    "built_rel_type",
    # Transfer-mode features (Lane 3 single-knob addition).
