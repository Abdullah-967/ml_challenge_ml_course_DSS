...
RECIPE_VARIANT = "deepen7_avg_area_per_room"
PARAMS_SUMMARY = (
    ...
    "geo+ratios+date+avg_area(per_apt,per_house)"
)
...
AVG_AREA_COLUMNS = ("avg_area_per_apt", "avg_area_per_house")
...
def add_avg_area_per_unit(frame):
    frame = frame.copy()
    apt_area = pd.to_numeric(frame.get("apartment_area"), errors="coerce")
    apt_count = pd.to_numeric(frame.get("num_apartments"), errors="coerce")
    house_area = pd.to_numeric(frame.get("house_area"), errors="coerce")
    house_count = pd.to_numeric(frame.get("num_houses"), errors="coerce")
    frame["avg_area_per_apt"] = (apt_area / apt_count.where(apt_count > 0, np.nan))
    frame["avg_area_per_house"] = (house_area / house_count.where(house_count > 0, np.nan))
    return frame
...
def infer_numeric_features(records):
    ...
    features.extend(AVG_AREA_COLUMNS)
    return features


def make_feature_frame(records, numeric_columns, categorical_columns, category_levels):
    frame = add_avg_area_per_unit(add_date_ordinal(add_derived_ratios(add_geo_features(pd.DataFrame(records)))))
    numeric_part = (
