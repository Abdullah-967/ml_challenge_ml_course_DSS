...
RECIPE_VARIANT = "deepen3_presence_flags"
PARAMS_SUMMARY = (
    ...
    "geo_signal+ratios+presence(has_apt,has_house,has_land,has_commercial,has_built)"
)
...
def add_presence_flags(frame):
    frame = frame.copy()
    frame["has_apt"] = gt_zero(frame.get("apartment_area"))
    frame["has_house"] = gt_zero(frame.get("house_area"))
    frame["has_land"] = gt_zero(frame.get("land_area"))
    frame["has_commercial"] = gt_zero(frame.get("num_commercial"))
    frame["has_built"] = gt_zero(frame.get("built_area"))
    return frame
...
def infer_numeric_features(records):
    ...
    features.extend(PRESENCE_FLAG_COLUMNS)
    return features


def make_feature_frame(records, numeric_columns, categorical_columns, category_levels):
    frame = add_presence_flags(add_derived_ratios(add_geo_features(pd.DataFrame(records))))
    numeric_part = (
