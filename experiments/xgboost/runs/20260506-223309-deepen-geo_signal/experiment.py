...
RECIPE_VARIANT = "deepen8_commune_target_enc"
PARAMS_SUMMARY = (
    ...
    "geo+ratios+date+commune_target_enc(smooth=20)"
)
...
COMMUNE_TARGET_ENCODING_SMOOTH = 20.0
TARGET_ENC_COLUMNS = ("commune_target_enc",)
...
def fit_commune_target_encoding(frame_with_geo, targets, smooth=COMMUNE_TARGET_ENCODING_SMOOTH):
    global_mean = float(np.mean(targets))
    df = pd.DataFrame({"commune": frame_with_geo["commune_first"], "target": targets})
    grouped = df.groupby("commune", dropna=False)["target"].agg(["mean", "count"])
    smoothed = (grouped["count"] * grouped["mean"] + smooth * global_mean) / (grouped["count"] + smooth)
    return {"global_mean": global_mean, "lookup": smoothed.to_dict()}


def apply_commune_target_encoding(frame_with_geo, encoding):
    series = frame_with_geo["commune_first"].map(encoding["lookup"])
    return series.fillna(encoding["global_mean"]).astype(np.float64)
...
def infer_numeric_features(records):
    ...
    features.extend(TARGET_ENC_COLUMNS)
    return features


def make_feature_frame(records, numeric_columns, categorical_columns, category_levels, target_encoding=None):
    frame = add_date_ordinal(add_derived_ratios(add_geo_features(pd.DataFrame(records))))
    if target_encoding is not None:
        frame["commune_target_enc"] = apply_commune_target_encoding(frame, target_encoding)
    numeric_part = (
...
def fit_predict(train_records, validation_records):
    ...
    geo_train_frame = add_geo_features(pd.DataFrame(train_records))
    target_encoding = fit_commune_target_encoding(geo_train_frame, y_train)

    train_frame = make_feature_frame(
        train_records, numeric_columns, CATEGORICAL_COLUMNS, category_levels, target_encoding=target_encoding
    )
    validation_frame = make_feature_frame(
        validation_records, numeric_columns, CATEGORICAL_COLUMNS, category_levels, target_encoding=target_encoding
    )
    model = build_model()
    if USE_LOG_TARGET:
