...
RECIPE_VARIANT = "deepen6_room_aggregates"
PARAMS_SUMMARY = (
    ...
    "geo+ratios+date+aggregates(total_rooms_apt,total_rooms_house,total_area_apt,total_area_house)"
)
...
ROOM_AGGREGATE_COLUMNS = (
    "total_rooms_apt",
    "total_rooms_house",
    "total_area_apt_check",
    "total_area_house_check",
)
...
def add_room_aggregates(frame):
    frame = frame.copy()
    apt_room_cols = ["num_apt_1_room", "num_apt_2_rooms", "num_apt_3_rooms", "num_apt_4_rooms", "num_apt_5plus_rooms"]
    house_room_cols = [
        "num_house_1_room",
        "num_house_2_rooms",
        "num_house_3_rooms",
    ...
    frame["total_rooms_apt"] = sum(pd.to_numeric(frame.get(c), errors="coerce").fillna(0) for c in apt_room_cols)
    frame["total_rooms_house"] = sum(pd.to_numeric(frame.get(c), errors="coerce").fillna(0) for c in house_room_cols)
    frame["total_area_apt_check"] = sum(pd.to_numeric(frame.get(c), errors="coerce").fillna(0) for c in apt_area_cols)
    frame["total_area_house_check"] = sum(pd.to_numeric(frame.get(c), errors="coerce").fillna(0) for c in house_area_cols)
    return frame
...
def infer_numeric_features(records):
    ...
    features.extend(ROOM_AGGREGATE_COLUMNS)
    return features


def make_feature_frame(records, numeric_columns, categorical_columns, category_levels):
    frame = add_room_aggregates(add_date_ordinal(add_derived_ratios(add_geo_features(pd.DataFrame(records)))))
    numeric_part = (
