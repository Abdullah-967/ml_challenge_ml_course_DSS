...
RECIPE_VARIANT = "v2_deepen_iter13_two_seed_avg"
PARAMS_SUMMARY = (
    "RandomForestRegressor(criterion=absolute_error,n_est=200,max_depth=18,"
    "min_samples_leaf=2,max_features=0.4,bootstrap=False,n_jobs=-1,random_state={42,2026}),"
    "log1p_target,smooth=10,built_rel_type,two_seed_avg"
)
RF_CRITERION = "absolute_error"
RF_N_ESTIMATORS = 200
RF_MAX_DEPTH = 18
TWO_SEED_AVERAGE_SEEDS = (42, 2026)
...
def add_built_rel_commune(train_frame, val_frame):
    mean_map, fallback = fit_group_mean(train_frame["commune_first"], train_frame.get("built_area"))
    train_frame = train_frame.copy()
    val_frame = val_frame.copy()
    train_frame["built_rel_commune"] = apply_group_relative(
        train_frame["commune_first"], train_frame.get("built_area"), mean_map, fallback
    )
    val_frame["built_rel_commune"] = apply_group_relative(
        val_frame["commune_first"], val_frame.get("built_area"), mean_map, fallback
    )
    return train_frame, val_frame
...
def build_model(numeric_columns, categorical_columns, n_estimators=RF_N_ESTIMATORS, random_state=42):
...
def fit_predict(train_records, validation_records):
    ...
    preds = []
    for seed in TWO_SEED_AVERAGE_SEEDS:
        model = build_model(numeric_columns, cat_cols, random_state=seed)
        model.fit(train_X, fit_target)
        preds.append(model.predict(val_X))
    averaged = np.mean(preds, axis=0)
    return np.expm1(averaged) if USE_LOG_TARGET else averaged
