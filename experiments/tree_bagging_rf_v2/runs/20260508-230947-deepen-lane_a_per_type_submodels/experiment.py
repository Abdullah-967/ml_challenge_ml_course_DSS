...
RECIPE_VARIANT = "v2_deepen_iter7_per_type_submodels"
PARAMS_SUMMARY = (
    ...
    "per_type_submodels(top3,n_est=100,blend=0.3)"
)
...
def fit_predict(train_records, validation_records):
    ...
    global_model = build_model(numeric_columns, cat_cols, n_estimators=RF_N_ESTIMATORS)
    fit_target = np.log1p(y_train) if USE_LOG_TARGET else y_train
    global_model.fit(train_X, fit_target)
    global_pred = global_model.predict(val_X)

    val_property = val_frame["property_type"].astype(object).where(val_frame["property_type"].notna(), MISSING_TOKEN)
    train_property = train_frame["property_type"].astype(object).where(train_frame["property_type"].notna(), MISSING_TOKEN)

    blended = global_pred.copy()
    for ptype in TOP_PROPERTY_TYPES:
        train_mask = (train_property == ptype).to_numpy()
        if train_mask.sum() < 100:
            continue
        sub_train_X = train_X.loc[train_mask]
        sub_target = fit_target[train_mask]
        sub_model = build_model(numeric_columns, cat_cols, n_estimators=SUB_MODEL_N_ESTIMATORS)
        sub_model.fit(sub_train_X, sub_target)
        val_mask = (val_property == ptype).to_numpy()
        if val_mask.any():
            sub_pred = sub_model.predict(val_X.loc[val_mask])
            blended[val_mask] = SUB_MODEL_BLEND * sub_pred + (1.0 - SUB_MODEL_BLEND) * global_pred[val_mask]

    return np.expm1(blended) if USE_LOG_TARGET else blended
