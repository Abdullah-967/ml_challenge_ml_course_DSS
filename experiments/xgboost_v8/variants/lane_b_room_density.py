"""xgboost_v8 smoke lane B -- add room-area-density ratios.

Anchor: xgboost_v7 promoted recipe (top15 sub-models, n_est=8000).
Single knob (feature, derived_ratios):
  ADD avg-area-per-room ratios for every apartment and house room bucket
  (10 cols total: 5 buckets x {apt, house}). hypothesis_unit:
  `room_area_density_ratios`. Each ratio is safe_div(area_X, num_X).
"""
...
RECIPE_VARIANT = "smoke_laneB_room_area_density_ratios"
PARAMS_SUMMARY = (
    ...
    "0.7*global+0.3*submodel(top10_types_min200,n_est=4000),"
    "+room_area_density_ratios(10cols),smoke_shrink"
)
DATE_EPOCH = pd.Timestamp("2014-01-01")
USE_LOG_TARGET = True

ROOM_BUCKETS = ("1_room", "2_rooms", "3_rooms", "4_rooms", "5plus_rooms")
ROOM_DENSITY_COLUMNS = tuple(
    f"avg_area_per_{kind}_{bucket}"
    for kind in ("apt", "house")
...
def add_room_area_density_ratios(frame):
    frame = frame.copy()
    for kind in ("apt", "house"):
        for bucket in ROOM_BUCKETS:
            area_col = f"area_{kind}_{bucket}"
            num_col = f"num_{kind}_{bucket}"
            frame[f"avg_area_per_{kind}_{bucket}"] = safe_div(frame.get(area_col), frame.get(num_col))
    return frame
...
def make_feature_frame_pre(records):
    return add_room_area_density_ratios(
        add_derived_ratios(
            add_date_ordinal(add_geo_first_tokens(pd.DataFrame(records)))
        )
    )
