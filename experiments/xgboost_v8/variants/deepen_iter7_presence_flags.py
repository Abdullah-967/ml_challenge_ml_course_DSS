"""xgboost_v8 deepen iter7 -- add presence flags.

Anchor: deepen iter6 (= iter2 family-best, log_skewed_numerics had no effect).
Single knob (feature, structural_numeric): ADD 5 binary presence indicators
(has_apartments, has_houses, has_commercial, has_dependencies, has_lots).
hypothesis_unit: `presence_flags`. Trees get explicit "transaction composition"
signal as a clean binary split rather than learning the implicit rule "count > 0".
"""
...
RECIPE_VARIANT = "deepen_iter7_presence_flags"
PARAMS_SUMMARY = (
