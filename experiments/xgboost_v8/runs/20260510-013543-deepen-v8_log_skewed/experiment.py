"""xgboost_v8 deepen iter6 -- log-transform skewed area numerics.

Anchor: deepen iter2 (family-best, v7-equivalent).
Single knob (preprocessing, structural_numeric): replace built_area, land_area,
house_area, apartment_area with their log1p variants in NUMERIC_COLUMNS while
keeping the original NaN structure (log1p of NaN is NaN). hypothesis_unit:
`log_skewed_numerics`. Trees with tree_method=hist discretize values into
fixed-bin histograms; log spacing maps the long-tailed area distribution onto
a more uniform bin density and may improve split-quality at depth=6.
"""
...
RECIPE_VARIANT = "deepen_iter6_log_skewed"
PARAMS_SUMMARY = (
    ...
    "log1p(built_area,land_area,house_area,apartment_area)"
)
DATE_EPOCH = pd.Timestamp("2014-01-01")
USE_LOG_TARGET = True
LOG_AREA_COLS = ("built_area", "house_area", "land_area", "apartment_area")
...
def materialize_features(frame, category_levels):
    ...
    # iter6 single knob: replace skewed area cols with log1p in-place AFTER materialize
    for col in LOG_AREA_COLS:
        if col in numeric_part.columns:
            numeric_part[col] = np.log1p(numeric_part[col].clip(lower=0))
