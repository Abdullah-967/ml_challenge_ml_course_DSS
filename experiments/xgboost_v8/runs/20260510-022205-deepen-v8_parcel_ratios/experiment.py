"""xgboost_v8 deepen iter8 -- add parcel_density_ratios.

Anchor: iter7 (presence_flags, family-best by tiny margin).
Single knob (feature, derived_ratios): ADD 4 cadastral density ratios:
  parcels_per_section, parcels_per_commune, sections_per_commune,
  lots_per_parcel. hypothesis_unit: `parcel_density_ratios`. Numerators and
  denominators come from raw count fields without adding them as numeric features.
"""
...
RECIPE_VARIANT = "deepen_iter8_parcel_ratios"
PARAMS_SUMMARY = (
    "XGBRegressor(reg:absoluteerror,n_est=8000,lr=0.05,max_depth=6,"
    "mcw=5,subsample=0.8,colsample=0.6,reg_lambda=1.0,hist,enable_categorical=True),"
    "0.7*global+0.3*submodel(top15_types_min200,n_est=8000),"
    "+presence_flags(5),+parcel_density_ratios(4)"
)
...
PARCEL_RATIO_COLS = (
    "parcels_per_section", "parcels_per_commune", "sections_per_commune", "lots_per_parcel",
)
...
def add_parcel_density_ratios(frame):
    frame = frame.copy()
    frame["parcels_per_section"] = safe_div(frame.get("num_parcels"), frame.get("num_sections"))
    frame["parcels_per_commune"] = safe_div(frame.get("num_parcels"), frame.get("num_communes"))
    frame["sections_per_commune"] = safe_div(frame.get("num_sections"), frame.get("num_communes"))
    frame["lots_per_parcel"] = safe_div(frame.get("num_lots"), frame.get("num_parcels"))
    return frame
...
def make_feature_frame_pre(records):
    return add_parcel_density_ratios(
        add_presence_flags(
