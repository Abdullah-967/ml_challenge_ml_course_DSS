"""xgboost_v8 smoke lane A -- add omitted cadastral counts.

Anchor: xgboost_v7 promoted recipe (top15 sub-models, n_est=8000).
Single knob (feature, structural_numeric):
  ADD num_parcels, num_sections, num_communes, num_dependencies to NUMERIC_COLUMNS.
These 4 cadastral/structural counts are present in dataset/features.json but
were never included in the v7 numeric column set. The hypothesis_unit is
`structural_numeric_baseline`: extend the broad structural-numeric scope to
fully cover dataset-provided count fields, before any deepen.
"""
...
RECIPE_VARIANT = "smoke_laneA_add_cadastral_counts"
PARAMS_SUMMARY = (
    ...
    "+num_parcels,num_sections,num_communes,num_dependencies,smoke_shrink"
)
...
NUMERIC_COLUMNS = (
    ...
    "built_rel_type",
    # lane A additions:
    "num_parcels", "num_sections", "num_communes", "num_dependencies",
)
CATEGORICAL_COLUMNS = (
