"""xgboost_v8 deepen iter9 -- add month_cyclical date encoding.

Anchor: iter7 (family-best, presence_flags).
Single knob (feature, date_time): ADD sin_month, cos_month derived from
transaction_date's month component. hypothesis_unit: `month_cyclical`. Trees
already have access to date_ordinal but cannot natively express seasonal
periodicity; sin/cos give them an explicit periodic signal as 2 numerics.
"""
...
RECIPE_VARIANT = "deepen_iter9_month_cyclical"
PARAMS_SUMMARY = (
    ...
    "+presence_flags(5),+sin_month,cos_month"
)
...
def add_date_features(frame):
    ...
    if "transaction_date" in frame.columns:
        ...
        frame["sin_month"] = np.sin(2.0 * np.pi * month / 12.0)
        frame["cos_month"] = np.cos(2.0 * np.pi * month / 12.0)
    else:
        frame["date_ordinal"] = np.nan
        frame["sin_month"] = np.nan
        frame["cos_month"] = np.nan
    return frame
...
def make_feature_frame_pre(records):
    return add_presence_flags(
        add_derived_ratios(add_date_features(add_geo_first_tokens(pd.DataFrame(records))))
    )
