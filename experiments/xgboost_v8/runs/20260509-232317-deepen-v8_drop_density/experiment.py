"""xgboost_v8 deepen iter2 -- drop room_area_density_ratios (discard direction).

Anchor: deepen iter1 (20260509-224734, lane B at restored capacity).
Single knob (feature, derived_ratios): DROP all 10 room_area_density_ratios
columns from NUMERIC_COLUMNS (and skip computing them). Tests whether the
+1733 regression vs v7 promoted is attributable to those 10 features.

Expected: cv_mae lands at ~46308 (v7 promoted reference), confirming density
ratios as the regressor. The pipeline otherwise matches v7 promoted exactly,
so iter2 also serves as a v8-family reproducibility check on v7's number.
"""
...
RECIPE_VARIANT = "deepen_iter2_drop_density"
PARAMS_SUMMARY = (
