"""xgboost_v11 reboot (axis E) -- shrunk v6 anchor + property_type x commune_first cross.

Single-knob change vs Lane 2 (shrunk_v6_anchor_n1000, 48772.08):
add one paired categorical feature `property_type_commune_first` =
"{property_type}|{commune_first}" as a native XGBoost categorical. The
parked velocity features (v11 smoke iter3) are removed so this is a clean
single-knob delta against the bare shrunk anchor.
"""
...
FEATURE_LANE = "shrunk_v6_anchor_n1000_plus_type_commune_cross"
RECIPE_VARIANT = "deepen_reboot_axis_e_type_commune_cross"
PARAMS_SUMMARY = (
    ...
    "+cat=property_type_commune_first"
)
...
def add_type_commune_cross(frame):
    frame = frame.copy()
    pt = frame.get("property_type", pd.Series([np.nan] * len(frame))).astype(str)
    cf = frame.get("commune_first", pd.Series([np.nan] * len(frame))).astype(str)
    pt = pt.where(pt.notna() & (pt != "nan") & (pt != ""), "MISSING")
    cf = cf.where(cf.notna() & (cf != "nan") & (cf != ""), "MISSING")
    frame["property_type_commune_first"] = pt + "|" + cf
    return frame
...
def make_feature_frame_pre(records):
    ...
    base = add_type_commune_cross(base)
    return base
...
def fit_category_levels(records):
    frame = add_type_commune_cross(add_geo_first_tokens(pd.DataFrame(records)))
    levels = {}
    for column in CATEGORICAL_COLUMNS:
