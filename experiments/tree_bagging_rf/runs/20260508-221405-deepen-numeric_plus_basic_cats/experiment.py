"""RandomForest round 2 deepen iter1 recipe.

Round 2 deepen iter1. Single-knob: add fold-safe built_rel_type =
built_area / mean(built_area | property_type) (anchor: ablate c2 best,
cv_mae 50663.64 seed=42). Hypothesis: tree models cannot easily compute
type-relative size ratios; explicit fold-safe group statistic should
inject typecat-relative information that raw structural numerics miss.
xgboost_v4 saw -245 from this exact unit.
"""
...
RECIPE_VARIANT = "round2_deepen_iter1_built_rel_type"
PARAMS_SUMMARY = (
...
def fit_group_mean(group_series, value_series):
    ...
    return df.groupby("g")["v"].mean().to_dict(), float(df["v"].mean()) if len(df) else 0.0
