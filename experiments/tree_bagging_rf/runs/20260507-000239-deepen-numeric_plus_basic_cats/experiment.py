...
RECIPE_VARIANT = "deepen_iter4_add_date_ordinal"
PARAMS_SUMMARY = (
    "RandomForestRegressor(n_est=200,max_depth=18,min_samples_leaf=2,"
    "max_features=sqrt,n_jobs=-1,random_state=42),"
    "all_numeric+ratios+OHE(property_type,transaction_type)+commune_te20+cadastral_te20+date_ordinal"
)
...
def add_target_encoding_columns(train_frame, val_frame, train_target):
    ...
    for src_col, out_col in (("commune_first", "commune_target_enc"), ("cadastral_first", "cadastral_target_enc")):
...
def prepare_frames(train_records, validation_records, train_target):
    train_frame = add_date_ordinal(add_derived_ratios(add_commune_first(pd.DataFrame(train_records))))
    val_frame = add_date_ordinal(add_derived_ratios(add_commune_first(pd.DataFrame(validation_records))))
