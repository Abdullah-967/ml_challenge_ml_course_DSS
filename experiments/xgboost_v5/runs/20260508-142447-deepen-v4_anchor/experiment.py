"""xgboost_v5 deepen iter5 -- single knob: 3-seed prediction averaging.

Anchor: v4 W''''' (smoke 1-3, deepen iter4 reverted).
This iter replaces single-seed predict with mean of seeds {42, 100, 200} for
both global and per-type sub-models. Variance reduction.

Distinct from v4 iter12 (3-seed at n_est=500): at n_est=4000 the per-fit
variance is higher (cv_mae_std ~5244 vs ~4922) so averaging headroom is larger.
"""
...
RECIPE_VARIANT = "deepen_iter5_3seeds"
PARAMS_SUMMARY = (
    ...
    "0.7*global+0.3*submodel(top5_types,n_est=4000),seeds={42,100,200}"
)
...
def fit_predict(train_records, validation_records):
    ...
    for seed in SEEDS:
        ...
        seed_predictions.append(final_pred)
