"""xgboost_v5 smoke Lane C -- single knob: commune_frequency hypothesis_unit.

Anchor: v4 W''''' promoted recipe.
This iter adds commune_first_freq = fold-safe count of commune_first level in
TRAIN fold (unseen commune in val => 0). Distinct from commune-level TE
(which failed v4 Lane C due to noisy 36k means) -- frequency measures market
liquidity, not mean price. Trees can use it to gate cat-derived signals.
"""
...
FEATURE_LANE = "v4_plus_commune_frequency"
RECIPE_VARIANT = "smoke_laneC_commune_frequency"
PARAMS_SUMMARY = (
    ...
    "0.7*global+0.3*submodel(top5_types,n_est=4000),+commune_first_freq(fold_safe)"
)
...
def compute_commune_freq_map(train_frame):
    series = train_frame["commune_first"].astype(str)
    series = series[series != "nan"]
    counts = series.value_counts()
    return counts.to_dict()


def add_commune_freq(frame, freq_map):
    frame = frame.copy()
    series = frame["commune_first"].astype(str)
    frame["commune_first_freq"] = series.map(freq_map).fillna(0).astype(float)
    return frame
...
def fit_predict(train_records, validation_records):
    ...
    freq_map = compute_commune_freq_map(train_pre)
    train_pre = add_commune_freq(train_pre, freq_map)
    val_pre = add_commune_freq(val_pre, freq_map)
