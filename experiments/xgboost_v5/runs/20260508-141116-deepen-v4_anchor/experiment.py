"""xgboost_v5 deepen iter4 -- single knob: cadastral_first_te hypothesis_unit.

Anchor: v4 W''''' (smoke lanes 1-3 all reverted).
This iter adds fold-safe cadastral_first TE smooth=20. Cadastral has ~846
levels, the untested middle ground between commune (36k, too noisy in v4
Lane C) and dept (96, tied in v4 iter2). Smoothing=20 keeps small-section
estimates anchored to global mean.
"""
...
RECIPE_VARIANT = "deepen_iter4_cadastral_te20"
PARAMS_SUMMARY = (
    "XGBRegressor(reg:absoluteerror,n_est=4000,lr=0.05,max_depth=6,"
    "mcw=5,subsample=0.8,colsample=0.6,reg_lambda=1.0,hist,enable_categorical=True),"
    "0.7*global+0.3*submodel(top5_types,n_est=4000),+cadastral_first_te(smooth=20)"
)
...
def compute_cadastral_te_map(train_frame, train_target_log, smooth=TE_SMOOTH):
    section = train_frame["cadastral_first"].astype(str)
    df = pd.DataFrame({"sec": section, "y": train_target_log})
    df = df[df["sec"] != "nan"]
    global_mean = float(df["y"].mean())
    grouped = df.groupby("sec")["y"]
    means = grouped.mean()
...
def add_cadastral_te(frame, te_map, global_mean):
    frame = frame.copy()
    section = frame["cadastral_first"].astype(str)
    frame["cadastral_first_te"] = section.map(te_map).fillna(global_mean).astype(float)
    return frame
...
def fit_predict(train_records, validation_records):
    ...
    te_map, te_global_mean = compute_cadastral_te_map(train_pre, y_train_log, smooth=TE_SMOOTH)
    train_pre = add_cadastral_te(train_pre, te_map, te_global_mean)
    val_pre = add_cadastral_te(val_pre, te_map, te_global_mean)
