"""xgboost_v5 smoke Lane B -- single knob: id_token_counts hypothesis_unit.

Anchor: v4 W''''' promoted recipe.
This iter adds two bounded-derived numeric features:
- num_parcel_ids: count of comma-separated tokens in parcel_ids
- num_transferred_parcel_ids: same for transferred_parcel_ids
Captures transaction scale/complexity orthogonal to area features. Raw IDs
themselves are NOT used as categoricals.
"""
...
FEATURE_LANE = "v4_plus_id_token_counts"
RECIPE_VARIANT = "smoke_laneB_id_token_counts"
PARAMS_SUMMARY = (
    ...
    "0.7*global+0.3*submodel(top5_types,n_est=4000),+id_token_counts(2)"
)
...
NUMERIC_COLUMNS = (
    ...
    "built_per_land",
    "built_rel_type",
    "num_parcel_ids", "num_transferred_parcel_ids",
)
CATEGORICAL_COLUMNS = (
...
def add_id_token_counts(frame):
    frame = frame.copy()
    if "parcel_ids" in frame.columns:
        frame["num_parcel_ids"] = frame["parcel_ids"].map(count_tokens)
    else:
        frame["num_parcel_ids"] = np.nan
    if "transferred_parcel_ids" in frame.columns:
        frame["num_transferred_parcel_ids"] = frame["transferred_parcel_ids"].map(count_tokens)
    else:
        frame["num_transferred_parcel_ids"] = np.nan
    return frame
