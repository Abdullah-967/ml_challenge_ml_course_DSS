"""xgboost_v5 smoke Lane A -- single knob: presence_flags hypothesis_unit.

Anchor: v4 W''''' promoted recipe.
This iter adds binary presence indicators for top-importance numeric fields:
is_present_built_area, is_present_house_area, is_present_land_area,
is_present_apartment_area, is_present_num_premises, is_present_num_houses,
is_present_num_apartments. Tests whether MISSINGNESS PATTERN carries signal
beyond xgboost's native missing handling.
"""
...
FEATURE_LANE = "v4_plus_presence_flags"
RECIPE_VARIANT = "smoke_laneA_presence_flags"
PARAMS_SUMMARY = (
    ...
    "0.7*global+0.3*submodel(top5_types,n_est=4000),+presence_flags(7)"
)
DATE_EPOCH = pd.Timestamp("2014-01-01")
USE_LOG_TARGET = True

PRESENCE_SOURCE_COLUMNS = (
    "built_area", "house_area", "land_area", "apartment_area",
    "num_premises", "num_houses", "num_apartments",
)
PRESENCE_FLAG_COLUMNS = tuple(f"is_present_{c}" for c in PRESENCE_SOURCE_COLUMNS)
...
def add_presence_flags(frame):
    frame = frame.copy()
    for col in PRESENCE_SOURCE_COLUMNS:
        if col in frame.columns:
            values = pd.to_numeric(frame[col], errors="coerce")
            frame[f"is_present_{col}"] = values.notna().astype(np.int8)
        else:
            frame[f"is_present_{col}"] = np.int8(0)
    return frame
