"""xgboost_v5 deepen iter6 -- single knob: per-region sub-models REPLACE per-type.

Anchor: v4 W''''' (smoke 1-3, deepen 4-5 reverted).
Tests if geographic specialization (per-region) captures more signal than
property-type specialization (per-type). Same cost: still 1 global + 5 sub fits.

Top 5 regions are computed from train data per fold (most frequent).
"""
...
RECIPE_VARIANT = "deepen_iter6_per_region_submodels"
PARAMS_SUMMARY = (
    ...
    "0.7*global+0.3*submodel(top5_regions,n_est=4000)"
)
...
def top_k_regions(records, k=5, min_count=100):
    series = pd.DataFrame(records).get("region_code", pd.Series([], dtype=object)).astype(str)
...
def fit_predict(train_records, validation_records):
    ...
    train_regions = pd.DataFrame(train_records).get("region_code", pd.Series([""] * len(train_records))).astype(str).values
    val_regions = pd.DataFrame(validation_records).get("region_code", pd.Series([""] * len(validation_records))).astype(str).values

    final_pred = pred_global.copy()
    BLEND = 0.3
    sub_regions = top_k_regions(train_records, k=5, min_count=100)
    for sub_region in sub_regions:
        train_mask = train_regions == sub_region
        val_mask = val_regions == sub_region
