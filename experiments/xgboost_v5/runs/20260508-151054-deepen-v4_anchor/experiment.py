"""xgboost_v5 deepen iter7 -- single knob: per-type sub-model max_depth=8.

Anchor: v4 W''''' (smoke 1-3, deepen 4-6 reverted).
Global model stays at max_depth=6. Per-type sub-models get max_depth=8 (vs 6).
Untested direction: v4 iter26 tested smaller (depth=4, tied); bigger sub-model
capacity may capture richer per-type interactions on top-5 type slices.
"""
...
RECIPE_VARIANT = "deepen_iter7_submodel_depth8"
PARAMS_SUMMARY = (
    ...
    "0.7*global+0.3*submodel(top5_types,n_est=4000,depth=8)"
)
