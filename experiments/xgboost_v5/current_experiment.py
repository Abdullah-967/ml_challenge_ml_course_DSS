"""xgboost_v5 anchor -- v4 W''''' promoted recipe (per-type sub-models + n_est=4000).

Carry-forward anchor: xgboost_v4 best_experiment.py
- features: v2 set + dept_code + region_code + built_per_land + built_rel_type
- target: log1p with expm1 inverse
- architecture: 0.7*global + 0.3*per-type sub-model (top 5 types)
- hyperparameters: n_est=4000, lr=0.05, max_depth=6, mcw=5, subsample=0.8,
  colsample_bytree=0.6, reg_lambda=1.0, hist, enable_categorical=True

Best result on this anchor:
- seed=42 cv_mae 47107.53 +/- 5244.67
- seed=2026 cv_mae 47998.49 +/- 3831.88
- 2-seed mean 47553

v5 single-knob iterations modify ONE knob from this anchor at a time.
"""
...
FEATURE_LANE = "v5_anchor_v4_promoted"
RECIPE_VARIANT = "v5_anchor"
PARAMS_SUMMARY = (
    ...
    "0.7*global+0.3*submodel(top5_types,n_est=4000)"
)
