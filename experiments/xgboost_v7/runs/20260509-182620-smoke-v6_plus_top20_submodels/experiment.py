"""xgboost_v7 smoke lane A -- top20 sub-model coverage.

Anchor: v6 promoted recipe (top15 sub-models, n_est=6000).
Single knob: top_k_types k=15/min_count=200 -> k=20/min_count=100.
"""
...
RECIPE_VARIANT = "smoke_laneA_top20_submodels_min100"
PARAMS_SUMMARY = (
    ...
    "0.7*global+0.3*submodel(top20_types_min100,n_est=6000)"
)
...
def top_k_types(records, k=20, min_count=100):
...
def fit_predict(train_records, validation_records):
    ...
    BLEND = 0.3
    sub_types = top_k_types(train_records, k=20, min_count=100)
