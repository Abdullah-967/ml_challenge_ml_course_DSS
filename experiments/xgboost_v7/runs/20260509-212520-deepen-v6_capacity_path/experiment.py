"""xgboost_v7 deepen iter2 -- internal early stopping capacity probe.

Anchor: v7 promoted capacity path (top15 sub-models, n_est=8000).
Single knob: replace fixed n_estimators=8000 with a 12000-tree ceiling and
fold-internal early stopping for both global and sub-models.
"""
...
MODEL_FAMILY = "xgboost_v7"
FEATURE_LANE = "v6_capacity_path"
RECIPE_VARIANT = "deepen_iter2_internal_early_stopping"
PARAMS_SUMMARY = (
    "XGBRegressor(reg:absoluteerror,n_est=12000,early_stopping_rounds=200,"
    "internal_eval=0.1,lr=0.05,max_depth=6,mcw=5,subsample=0.8,"
    "colsample=0.6,reg_lambda=1.0,hist,enable_categorical=True),"
    "0.7*global+0.3*submodel(top15_types_min200,n_est=12000,early_stop=200)"
)
DATE_EPOCH = pd.Timestamp("2014-01-01")
USE_LOG_TARGET = True
N_ESTIMATORS = 12000
EARLY_STOPPING_ROUNDS = 200
INTERNAL_EVAL_FRACTION = 0.1
INTERNAL_EVAL_RANDOM_STATE = 2027
...
def internal_eval_split(n_rows, random_state, eval_fraction=INTERNAL_EVAL_FRACTION):
    indices = np.arange(n_rows)
    rng = np.random.default_rng(random_state)
    rng.shuffle(indices)
    eval_size = int(round(n_rows * eval_fraction))
    eval_size = min(max(eval_size, 1), n_rows - 1)
    eval_idx = indices[:eval_size]
    fit_idx = indices[eval_size:]
    return fit_idx, eval_idx


def fit_model_with_internal_eval(model, frame, target, random_state):
    fit_idx, eval_idx = internal_eval_split(len(frame), random_state=random_state)
    model.fit(
        frame.iloc[fit_idx],
        target[fit_idx],
        eval_set=[(frame.iloc[eval_idx], target[eval_idx])],
        verbose=False,
    )
    return model
...
def top_k_types(records, k=15, min_count=200):
...
def fit_predict(train_records, validation_records):
    ...
    fit_model_with_internal_eval(
        ...
        train_frame,
        y_train_log,
        random_state=INTERNAL_EVAL_RANDOM_STATE,
    )
    ...
    BLEND = 0.3
    sub_types = top_k_types(train_records, k=15, min_count=200)
    for sub_type in sub_types:
        ...
        seed_offset = sum(ord(ch) for ch in sub_type)
        fit_model_with_internal_eval(
            sub_model,
            sub_train_frame,
            sub_y,
            random_state=INTERNAL_EVAL_RANDOM_STATE + seed_offset,
        )
