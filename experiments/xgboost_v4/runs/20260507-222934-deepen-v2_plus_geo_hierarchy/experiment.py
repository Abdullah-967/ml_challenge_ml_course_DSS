...
RECIPE_VARIANT = "deepen_iter17_early_stopping"
PARAMS_SUMMARY = (
    ...
    "early_stopping_rounds=30,internal_eval_split=0.1),"
    "W''+early_stopping"
)
...
def fit_predict(train_records, validation_records):
    ...
    n = len(y_train)
    eval_size = max(int(0.1 * n), 100)
    perm = rng.permutation(n)
    eval_idx = perm[:eval_size]
    inner_train_idx = perm[eval_size:]
    train_inner = train_frame.iloc[inner_train_idx]
    train_eval = train_frame.iloc[eval_idx]
    y_inner = y_train[inner_train_idx]
    y_eval = y_train[eval_idx]

    model = build_model()
    if USE_LOG_TARGET:
        model.fit(train_inner, np.log1p(y_inner), eval_set=[(train_eval, np.log1p(y_eval))], verbose=False)
        return np.expm1(model.predict(val_frame))
    model.fit(train_inner, y_inner, eval_set=[(train_eval, y_eval)], verbose=False)
