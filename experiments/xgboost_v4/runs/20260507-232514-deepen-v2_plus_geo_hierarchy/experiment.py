...
RECIPE_VARIANT = "deepen_iter28_per_type_log_residual"
PARAMS_SUMMARY = (
    ...
    "global_log+per_type_log_residual_submodel(top5_types)"
)
...
def fit_predict(train_records, validation_records):
    ...
    # Compute log-pred from global on train, then log-residuals
    train_pred_log_global = global_model.predict(train_frame)
    val_pred_log_global = global_model.predict(val_frame)
    train_residuals_log = y_train_log - train_pred_log_global
    val_pred_log_total = val_pred_log_global.copy()
    ...
    for sub_type in SUB_TYPES:
        ...
        sub_y_log_residual = train_residuals_log[train_mask]
        sub_model = build_model()
        sub_model.fit(sub_train_frame, sub_y_log_residual)
        sub_residual_log_pred = sub_model.predict(val_frame[val_mask])
        val_pred_log_total[val_mask] = val_pred_log_global[val_mask] + sub_residual_log_pred
    return np.expm1(val_pred_log_total)
