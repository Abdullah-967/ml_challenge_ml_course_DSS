...
RECIPE_VARIANT = "deepen_iter14_blend_log_raw"
PARAMS_SUMMARY = (
...
def fit_predict(train_records, validation_records):
    ...
    model_log.fit(train_frame, np.log1p(y_train))
    pred_log = np.expm1(model_log.predict(val_frame))

    model_raw = build_model()
    model_raw.fit(train_frame, y_train)
    pred_raw = model_raw.predict(val_frame)

    return 0.5 * pred_log + 0.5 * pred_raw
