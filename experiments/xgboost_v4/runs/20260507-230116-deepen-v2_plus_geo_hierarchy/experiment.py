...
RECIPE_VARIANT = "deepen_iter21_te_type_x_dept"
PARAMS_SUMMARY = (
...
def fit_predict(train_records, validation_records):
    ...
    # 2D fold-safe TE on (property_type, dept_code) with smoothing=30
    y_train_log_full = np.log1p(make_target_array(train_records))
    te2d_df = pd.DataFrame({
        "type": train_pre["property_type"].astype(str),
        "dept": train_pre["dept_code"].astype(str),
        "y": y_train_log_full,
    })
    te2d_df = te2d_df[(te2d_df["type"] != "nan") & (te2d_df["dept"] != "nan")]
    global_mean_log = te2d_df["y"].mean()
    grouped = te2d_df.groupby(["type", "dept"])["y"]
    counts = grouped.count()
    means = grouped.mean()
    smoothing_2d = 30.0
    smoothed = (counts * means + smoothing_2d * global_mean_log) / (counts + smoothing_2d)
    te2d_map = smoothed.to_dict()  # keys are (type, dept) tuples
    for fr in (train_pre, val_pre):
        type_str = fr["property_type"].astype(str).fillna("nan")
        dept_str = fr["dept_code"].astype(str).fillna("nan")
        fr["te_type_x_dept"] = [
            te2d_map.get((t, d), global_mean_log)
            for t, d in zip(type_str, dept_str)
        ]
