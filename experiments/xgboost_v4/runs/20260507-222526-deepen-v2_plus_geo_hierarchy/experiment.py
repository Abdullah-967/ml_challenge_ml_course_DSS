...
RECIPE_VARIANT = "deepen_iter15_type_mean_built"
PARAMS_SUMMARY = (
...
NUMERIC_COLUMNS = (
    ...
    "built_per_land",
    "built_rel_type", "type_mean_built",
)
CATEGORICAL_COLUMNS = (
...
def fit_predict(train_records, validation_records):
    ...
    # type_mean_built as direct feature (per-row mapped)
    for fr in (train_pre, val_pre):
        type_str = fr["property_type"].astype(str)
        fr["type_mean_built"] = type_str.map(type_means).fillna(global_mean).astype(float)
