"""xgboost_v4 smoke Lane C -- v2 base + fold-safe TE on commune_first (smooth=30).

The TE is computed strictly from train_records inside fit_predict(), then mapped
onto validation_records. Since fit_predict is called once per CV fold with
disjoint (train, validation) splits, this is leakage-safe.
"""
...
RECIPE_VARIANT = "smoke_laneC_fold_te30"
PARAMS_SUMMARY = (
    ...
    "v2_base+fold_safe_commune_te30"
)
...
def compute_commune_te(train_frame, train_log_target, smoothing):
    """Smoothed mean log1p target per commune_first, computed on train only."""
    df = pd.DataFrame({
        "commune": train_frame["commune_first"].astype(str),
        "y": train_log_target,
    })
    df = df[df["commune"] != "nan"]
    global_mean = df["y"].mean()
    grouped = df.groupby("commune")["y"]
...
def add_te_column(frame, te_map, global_mean):
    frame = frame.copy()
    frame["te_commune_first"] = (
        frame["commune_first"].astype(str).map(te_map).fillna(global_mean).astype(float)
    )
    return frame


def make_feature_frame_no_te(records):
...
def fit_predict(train_records, validation_records):
    ...
    train_frame_pre = make_feature_frame_no_te(train_records)
    validation_frame_pre = make_feature_frame_no_te(validation_records)

    y_train = make_target_array(train_records)
    y_train_log = np.log1p(y_train)

    te_map, global_mean = compute_commune_te(train_frame_pre, y_train_log, TE_SMOOTHING)
    train_frame_pre = add_te_column(train_frame_pre, te_map, global_mean)
    validation_frame_pre = add_te_column(validation_frame_pre, te_map, global_mean)

    train_frame = materialize_features(train_frame_pre, category_levels)
    validation_frame = materialize_features(validation_frame_pre, category_levels)
