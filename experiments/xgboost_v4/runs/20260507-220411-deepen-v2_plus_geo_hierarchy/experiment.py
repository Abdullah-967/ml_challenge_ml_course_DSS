"""xgboost_v4 smoke Lane A -- v2 base + dept_code + region_code native cats.

Tests whether geographic hierarchy (department, region) adds signal above the
commune_first cat already in v2's recipe.
"""
...
RECIPE_VARIANT = "deepen_iter4_total_rooms"
PARAMS_SUMMARY = (
...
def add_derived_ratios(frame):
    ...
    room_columns = [
        "num_apt_1_room", "num_apt_2_rooms", "num_apt_3_rooms", "num_apt_4_rooms", "num_apt_5plus_rooms",
        "num_house_1_room", "num_house_2_rooms", "num_house_3_rooms", "num_house_4_rooms", "num_house_5plus_rooms",
    ]
    available = [c for c in room_columns if c in frame.columns]
    if available:
        frame["total_rooms"] = frame[available].apply(pd.to_numeric, errors="coerce").fillna(0).sum(axis=1)
    else:
        frame["total_rooms"] = np.nan
    return frame
