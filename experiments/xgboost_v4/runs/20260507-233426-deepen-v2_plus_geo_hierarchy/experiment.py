...
RECIPE_VARIANT = "deepen_iter32_global_5bag"
PARAMS_SUMMARY = (
    ...
    "0.7*5bag_global+0.3*submodel(top5_types)"
)
...
def fit_predict(train_records, validation_records):
    ...
    # Global = 5-bag bootstrap ensemble
    n_train = len(train_frame)
    rng = np.random.default_rng(42)
    bag_preds_val = []
    for bag_idx in range(5):
        idx = rng.integers(0, n_train, size=n_train)
        bag_train = train_frame.iloc[idx]
        bag_y_log = y_train_log[idx]
        bag_model = XGBRegressor(
            objective="reg:absoluteerror",
            n_estimators=500,
            learning_rate=0.05,
            max_depth=6,
        ...
        bag_model.fit(bag_train, bag_y_log)
        bag_preds_val.append(np.expm1(bag_model.predict(val_frame)))
    pred_global = np.mean(np.stack(bag_preds_val, axis=0), axis=0)
    # Train one full-data global for sub-model residual computation (fast since per-type sub-models train on full data later)
