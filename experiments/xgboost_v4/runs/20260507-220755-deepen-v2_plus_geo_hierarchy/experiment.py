...
RECIPE_VARIANT = "deepen_iter6_built_rel_commune"
PARAMS_SUMMARY = (
...
NUMERIC_COLUMNS = (
    ...
    "built_per_land",
    "built_rel_type", "built_rel_commune",
)
CATEGORICAL_COLUMNS = (
...
def compute_group_mean_map(train_frame, key_column, value_column):
    df = pd.DataFrame({
        "key": train_frame[key_column].astype(str),
        "val": pd.to_numeric(train_frame.get(value_column), errors="coerce"),
    })
    df = df[(df["key"] != "nan") & df["val"].notna() & (df["val"] > 0)]
    means = df.groupby("key")["val"].mean()
    global_mean = df["val"].mean()
    return means.to_dict(), global_mean


def add_relative_feature(frame, key_column, value_column, output_column, key_means, global_mean):
    frame = frame.copy()
    key_str = frame[key_column].astype(str)
    mean_per_row = key_str.map(key_means).fillna(global_mean).astype(float)
    val = pd.to_numeric(frame.get(value_column), errors="coerce")
    frame[output_column] = val / mean_per_row.replace(0, np.nan)
    return frame
...
def fit_predict(train_records, validation_records):
    ...
    type_means, type_global = compute_group_mean_map(train_pre, "property_type", "built_area")
    train_pre = add_relative_feature(train_pre, "property_type", "built_area", "built_rel_type", type_means, type_global)
    val_pre = add_relative_feature(val_pre, "property_type", "built_area", "built_rel_type", type_means, type_global)

    commune_means, commune_global = compute_group_mean_map(train_pre, "commune_first", "built_area")
    train_pre = add_relative_feature(train_pre, "commune_first", "built_area", "built_rel_commune", commune_means, commune_global)
    val_pre = add_relative_feature(val_pre, "commune_first", "built_area", "built_rel_commune", commune_means, commune_global)
    ...
    if USE_LOG_TARGET:
        ...
        return np.expm1(model.predict(val_frame))
