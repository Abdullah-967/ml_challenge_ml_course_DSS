...
RECIPE_VARIANT = "deepen_iter27_per_type_residual"
PARAMS_SUMMARY = (
    ...
    "global+per_type_residual_submodel(top5_types)"
)
...
def fit_predict(train_records, validation_records):
    ...
    # Global model + train predictions for residual stacking
    global_model = build_model()
    global_model.fit(train_frame, y_train_log)
    train_pred_global = np.expm1(global_model.predict(train_frame))
    ...
    train_residuals = y_train - train_pred_global
    for sub_type in SUB_TYPES:
        ...
        sub_y_residual = train_residuals[train_mask]
        sub_model = build_model()
        sub_model.fit(sub_train_frame, sub_y_residual)
        sub_residual_pred = sub_model.predict(val_frame[val_mask])
        final_pred[val_mask] = pred_global[val_mask] + sub_residual_pred
    return final_pred
