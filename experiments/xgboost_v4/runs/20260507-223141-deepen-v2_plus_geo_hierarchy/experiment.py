...
RECIPE_VARIANT = "deepen_iter18_hetero_ensemble_5"
PARAMS_SUMMARY = (
    "XGBRegressor(reg:absoluteerror,hist,enable_categorical=True),"
    "5-model_hetero(depth_4_to_8,lr_0.03_to_0.1,subsample_0.6_to_0.8)"
)
...
ENSEMBLE_CONFIGS = (
    {"max_depth": 4, "lr": 0.05, "n_est": 500, "subsample": 0.8, "colsample": 0.6, "seed": 42},
    {"max_depth": 6, "lr": 0.05, "n_est": 500, "subsample": 0.8, "colsample": 0.6, "seed": 42},
    {"max_depth": 8, "lr": 0.05, "n_est": 500, "subsample": 0.6, "colsample": 0.6, "seed": 100},
    {"max_depth": 6, "lr": 0.03, "n_est": 1000, "subsample": 0.8, "colsample": 0.6, "seed": 200},
    {"max_depth": 6, "lr": 0.10, "n_est": 200, "subsample": 0.8, "colsample": 0.6, "seed": 300},
)
...
def fit_predict(train_records, validation_records):
    ...
    for cfg in ENSEMBLE_CONFIGS:
        ...
        if USE_LOG_TARGET:
            ...
            preds.append(np.expm1(model.predict(val_frame)))
        else:
            model.fit(train_frame, y_train)
            preds.append(model.predict(val_frame))
