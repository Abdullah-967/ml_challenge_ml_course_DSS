...
RECIPE_VARIANT = "deepen_iter9_proptype_te20"
PARAMS_SUMMARY = (
...
NUMERIC_COLUMNS = (
    ...
    "built_per_land",
    "built_rel_type", "te_property_type",
)
CATEGORICAL_COLUMNS = (
...
def fit_predict(train_records, validation_records):
    ...
    # property_type fold-safe TE (smooth=20) on log1p target
    y_train_log = np.log1p(make_target_array(train_records))
    te_df = pd.DataFrame({
        "type": train_pre["property_type"].astype(str),
        "y": y_train_log,
    })
    te_df = te_df[te_df["type"] != "nan"]
    te_global = te_df["y"].mean()
    te_grouped = te_df.groupby("type")["y"]
    te_counts = te_grouped.count()
    te_means = te_grouped.mean()
    te_smoothing = 20.0
    te_smoothed = (te_counts * te_means + te_smoothing * te_global) / (te_counts + te_smoothing)
    te_map = te_smoothed.to_dict()
    for fr in (train_pre, val_pre):
        type_str = fr["property_type"].astype(str)
        fr["te_property_type"] = type_str.map(te_map).fillna(te_global).astype(float)
