"""xgboost_v4 smoke Lane B -- v2 base + year (cat) + month (cat) + future_sale.

Tests temporal granularity beyond v2's date_ordinal continuous feature.
"""
...
RECIPE_VARIANT = "smoke_laneB_temporal_cats"
PARAMS_SUMMARY = (
    ...
    "v2_base+year_cat+month_cat+future_sale"
)
...
def add_temporal_cats(frame):
    frame = frame.copy()
    if "year" in frame.columns:
        frame["year_str"] = frame["year"].astype("Int64").astype(str)
    else:
        frame["year_str"] = np.nan
    if "month" in frame.columns:
        frame["month_str"] = frame["month"].astype("Int64").astype(str)
    else:
        frame["month_str"] = np.nan
    if "future_sale" in frame.columns:
        frame["future_sale_int"] = frame["future_sale"].astype("Int64")
    else:
        frame["future_sale_int"] = np.nan
    return frame
...
def fit_category_levels(records):
    frame = add_temporal_cats(add_geo_first_tokens(pd.DataFrame(records)))
    levels = {}
    for column in CATEGORICAL_COLUMNS:
