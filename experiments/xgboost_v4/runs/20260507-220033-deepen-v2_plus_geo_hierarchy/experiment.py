"""xgboost_v4 deepen iter2 -- single knob: fold-safe dept-level TE (smooth=20).

Carry-forward W: smoke Lane A (cv 49112).
Adds te_dept = smoothed mean log1p(property_value) per dept_code, computed on
the train fold only inside fit_predict (fold-safe).
"""
...
RECIPE_VARIANT = "deepen_iter2_dept_te20"
PARAMS_SUMMARY = (
...
def compute_te(train_frame, train_log_target, key_column, smoothing):
    df = pd.DataFrame({
...
def add_te_dept(frame, te_map, global_mean):
    frame = frame.copy()
    frame["te_dept"] = (
        frame["dept_code"].astype(str).map(te_map).fillna(global_mean).astype(float)
    )
    return frame
...
def fit_predict(train_records, validation_records):
    ...
    te_map, global_mean = compute_te(train_pre, y_train_log, "dept_code", TE_SMOOTHING)
    train_pre = add_te_dept(train_pre, te_map, global_mean)
    val_pre = add_te_dept(val_pre, te_map, global_mean)
