...
RECIPE_VARIANT = "deepen_iter11_house_rel_type"
PARAMS_SUMMARY = (
...
def fit_predict(train_records, validation_records):
    ...
    # house_area_rel_type fold-safe
    house_df = pd.DataFrame({
        "type": train_pre["property_type"].astype(str),
        "ha": pd.to_numeric(train_pre.get("house_area"), errors="coerce"),
    })
    house_df = house_df[(house_df["type"] != "nan") & house_df["ha"].notna() & (house_df["ha"] > 0)]
    house_means = house_df.groupby("type")["ha"].mean().to_dict()
    house_global = house_df["ha"].mean() if not house_df.empty else 1.0
    for fr in (train_pre, val_pre):
        type_str = fr["property_type"].astype(str)
        mean_per_row = type_str.map(house_means).fillna(house_global).astype(float)
        ha_val = pd.to_numeric(fr.get("house_area"), errors="coerce")
        fr["house_rel_type"] = ha_val / mean_per_row.replace(0, np.nan)
