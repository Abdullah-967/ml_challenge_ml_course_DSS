...
RECIPE_VARIANT = "deepen_iter8_land_rel_type"
PARAMS_SUMMARY = (
...
def fit_predict(train_records, validation_records):
    ...
    # land_rel_type: land_area / mean(land_area | property_type)
    land_df = pd.DataFrame({
        "type": train_pre["property_type"].astype(str),
        "land": pd.to_numeric(train_pre.get("land_area"), errors="coerce"),
    })
    land_df = land_df[(land_df["type"] != "nan") & land_df["land"].notna() & (land_df["land"] > 0)]
    land_means = land_df.groupby("type")["land"].mean()
    land_global = land_df["land"].mean()
    for fr in (train_pre, val_pre):
        type_str = fr["property_type"].astype(str)
        mean_per_row = type_str.map(land_means.to_dict()).fillna(land_global).astype(float)
        land_val = pd.to_numeric(fr.get("land_area"), errors="coerce")
        fr["land_rel_type"] = land_val / mean_per_row.replace(0, np.nan)
