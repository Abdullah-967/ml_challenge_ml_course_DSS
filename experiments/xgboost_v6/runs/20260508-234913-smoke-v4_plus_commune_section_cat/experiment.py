"""xgboost_v6 smoke lane C -- v4 W''''' plus nested commune-section category.

Single knob: add commune_section = commune_first + cadastral_first. Section
codes repeat across communes, so the combined category tests whether nested
cadastral geography adds signal beyond separate native categorical splits.
"""
...
RECIPE_VARIANT = "smoke_laneC_commune_section_cat"
PARAMS_SUMMARY = (
...
CATEGORICAL_COLUMNS = (
    ...
    "dept_code", "region_code", "commune_section",
)
...
def add_geo_first_tokens(frame):
    ...
    commune = frame.get("commune_first", pd.Series([np.nan] * len(frame), index=frame.index)).astype(str)
    section = frame.get("cadastral_first", pd.Series([np.nan] * len(frame), index=frame.index)).astype(str)
    frame["commune_section"] = commune + "_" + section
    return frame
