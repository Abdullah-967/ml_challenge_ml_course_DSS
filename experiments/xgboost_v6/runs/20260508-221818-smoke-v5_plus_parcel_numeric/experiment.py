"""xgboost_v6 smoke lane A -- v5 iter10 anchor plus parcel numeric components.

Single knob: add bounded numeric components parsed from the first parcel_ids
token. This avoids raw high-cardinality IDs while testing whether within-section
parcel numbering carries spatial signal beyond commune/section categories.
"""
...
MODEL_FAMILY = "xgboost_v6"
FEATURE_LANE = "v5_plus_parcel_numeric"
RECIPE_VARIANT = "smoke_laneA_parcel_numeric_components"
PARAMS_SUMMARY = (
    "XGBRegressor(reg:absoluteerror,n_est=5000,lr=0.05,max_depth=6,"
    "mcw=5,subsample=0.8,colsample=0.6,reg_lambda=1.0,hist,enable_categorical=True),"
    "0.7*global+0.3*submodel(top10_types_min200,n_est=5000)"
)
...
NUMERIC_COLUMNS = (
    ...
    "built_per_land",
    "built_rel_type",
    "parcel_id_prefix_code", "parcel_id_number",
)
CATEGORICAL_COLUMNS = (
...
def add_parcel_numeric_components(frame):
    frame = frame.copy()
    if "parcel_ids" not in frame.columns:
        frame["parcel_id_prefix_code"] = np.nan
        frame["parcel_id_number"] = np.nan
        return frame

    first_parcel = frame["parcel_ids"].map(first_token).astype(str)
    frame["parcel_id_prefix_code"] = pd.to_numeric(first_parcel.str.slice(5, 8), errors="coerce")
    frame["parcel_id_number"] = pd.to_numeric(first_parcel.str.slice(-4), errors="coerce")
    return frame
...
def make_feature_frame_pre(records):
    return add_derived_ratios(add_date_ordinal(add_parcel_numeric_components(add_geo_first_tokens(pd.DataFrame(records)))))
...
def top_k_types(records, k=10, min_count=200):
...
def fit_predict(train_records, validation_records):
    ...
    BLEND = 0.3
    sub_types = top_k_types(train_records, k=10, min_count=200)
